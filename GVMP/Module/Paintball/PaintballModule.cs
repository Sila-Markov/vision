...
namespace GVMP
{
    class PaintballModule : GVMP.Module.Module<PaintballModule>
    {
        ...
        protected override bool OnLoad()
        {
            ...
            NAPI.Blip.CreateBlip(432, new Vector3(434.36, -645.99, 28.73), 1.0f, 0, "Paintball", 255, 0, true, 0, 0);
            NAPI.Marker.CreateMarker(1, new Vector3(434.36, -645.99, 28.73), new Vector3(), new Vector3(), 1.0f, new Color(255, 140, 0), false, 0);

            ColShape cb = NAPI.ColShape.CreateCylinderColShape(new Vector3(434.36, -645.99, 28.73), 1.4f, 1.4f, 0);
        ...
        public static void PaintballDeath(DbPlayer dbPlayer, DbPlayer dbPlayer2)
        {
            try
            {
                ...
                PaintballModel paintballModel = dbPlayer.GetData("PBZone");
                int newdeaths = 1;
                if (!dbPlayer.HasData("PBDeaths") || dbPlayer.GetData("PBDeaths") == null)
                {
                    dbPlayer.SetData("PBDeaths", 1);
                }
                else
                {
                    newdeaths = dbPlayer.GetData("PBDeaths");
                    newdeaths += 1;
                }
