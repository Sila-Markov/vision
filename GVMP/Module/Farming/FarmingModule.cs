    ...
    class FarmingModule : GVMP.Module.Module<FarmingModule>
    {
        ...
        protected override bool OnLoad()
        {
            ...
            farming.Add(new FarmingModel
            {
                ...
                Item_1 = "Gummi",
                Item_2 = "GoldenDildo",
                Sammler = new Vector3(1507.42, -2146.72, 77.09),
                PickupCount = 1,
                Verarbeiter = new Vector3(780.9, 1275.3, 361.28),
                RemoveCount = 250,
                AddCount = 1,
                ColShapeRange = 30
            });*/
        ...
        public void showDrugMenu(Client c)
        {
            try
            {
                ...
                nativeItems.Add(new NativeItem("1x Golden Dildo - 100.000.000$", "GoldenDildo"));
        ...
        public void DrogenDealer(Client c, string value)
        {
            try
            {
                ...
                {
                    if (dbPlayer.GetItemAmount("GoldenDildo") >= 1)
                    {
        ...
        [RemoteEvent("showGummiMenu")]
        public void showGummiMenu(Client c)
        {
            try
            {
                if (c == null) return;
                ...
                nativeItems.Add(new NativeItem("100x Gummi - 25.000.000$", "Gummi"));
                nativeItems.Add(new NativeItem("1x Golden Dildo - 80.000.000$", "GoldenDildo"));

                NativeMenu nativeMenu = new NativeMenu("MarkiRevo", "", nativeItems);
                dbPlayer.ShowNativeMenu(nativeMenu);
            }
            catch (Exception ex)
            {
                Logger.Print("[EXCEPTION showMarkiRevoMenu] " + ex.Message);
                Logger.Print("[EXCEPTION showMarkiRevoMenu] " + ex.StackTrace);
            }
        }

        [RemoteEvent("nM-MarkiRevo")]
        public void MarkiRevo(Client c, string value)
        {
            try
            {
                DbPlayer dbPlayer = c.GetPlayer();
            ...
            {
                Logger.Print("[EXCEPTION MarkiRevo] " + ex.Message);
                Logger.Print("[EXCEPTION MarkiRevo] " + ex.StackTrace);
            }
        }
    }
}
