    ...
    public static class MySqlManager
    {
		public static Vector3 GetDbLocation(this DbPlayer dbPlayer)
		{
			...
			try
			{
				...
				try
				{
					if (((DbDataReader)(object)reader).HasRows && ((DbDataReader)(object)reader).Read())
					{
        ...
        internal static IEnumerable<GarageVehicle> GetAllVehicles() => throw new NotImplementedException();
		...
		public static void UpdateBusinessItems(this Business business, ItemModel item, bool remove)
		{
			...
			try
			{
				Item item4 = ItemModule.itemRegisterList.Find((Item item3) => item3.Name == item.Name);
				if (item4 == null)
				{
					return;
				}
				...
				try
				{
					try
					{
						...
						if (remove)
						{
							ItemModel itemModel = businessItems.Find((ItemModel item) => item.Name == item.Name);
							if (itemModel == null)
							{
								return;
							}
			...
			try
			{
				...
				try
				{
					try
					{
						...
						if (remove)
						{
							ItemModel itemModel = houseItems.Find((ItemModel item) => item.Name == item.Name);
							if (itemModel == null)
							{
								return;
							}
		...
		public static void UpdateLaborStorageItems(this Laboratory laboratory, int Id, ItemModel item, bool remove)
		{
			try
			{
				...
				if (remove)
				{
					ItemModel itemModel = list.Find((ItemModel item) => item.Name == item.Name);
					if (itemModel == null)
					{
						return;
					}
			...
			try
			{
				...
				try
				{
					try
					{
						...
						if (remove)
						{
							ItemModel itemModel = factionStorageItems.Find((ItemModel item) => item.Name == item.Name);
							if (itemModel == null)
							{
								return;
							}
		...
		public static void UpdateInventoryItems(this DbPlayer dbPlayer, string name, int amount, bool remove)
		{
			...
			try
			{
				Item item4 = ItemModule.itemRegisterList.Find((Item item3) => item3.Name == name);
				if (item4 == null)
				{
					return;
				}
				...
				try
				{
					try
					{
						...
						ItemModel itemModel = inventoryItems.Find((ItemModel model2) => model2.Name == name);
						bool flag = false;
						if (itemModel == null)
						{
							if (remove)
		...
		{
			return dbPlayer.GetInventoryItems().Find((ItemModel itemModel) => itemModel.Name == ItemName)?.Amount ?? 0;
		}
