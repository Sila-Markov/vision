...
namespace GVMP
{
    public class MAZ1 : GVMP.Module.Module<MAZ1>
    {
        public static List<MAZ1> clothingList = new List<MAZ1>();
        public static List<MAZ1> BlockedZones = new List<MAZ1>();
        private static MAZ1 awd;

        public static object mySqlReaderCon { get; private set; }

        protected override bool OnLoad()
        {
            Vector3 Position = new Vector3(-2162.23, 1321.99, 258.32);

            ColShape val = NAPI.ColShape.CreateCylinderColShape(Position, 1.4f, 2.4f, 0);
            val.SetData("FUNCTION_MODEL", new FunctionModel("openMAZ1"));
        ...
        [RemoteEvent("openMAZ1")]
        public static void openMAZ1(Client client)
        {
            try
            {
                ...
                if (!dbPlayer.IsFarming)
                {
                    ...
                    dbPlayer.disableAllPlayerActions(true);
                        ...
                        Notification.SendGlobalNotification("Zentrales Flugabwehrsystem: Das Flugzeug wird aufgebrochen (Nähe Labor Yakuza)!", 10000, "white", Notification.icon.bullhorn);
                    ...
                    {
                        Logger.Print("[EXCEPTION openMAZ1] " + ex.Message);
                        Logger.Print("[EXCEPTION openMAZ1] " + ex.StackTrace);
            }
        }
    }
}
