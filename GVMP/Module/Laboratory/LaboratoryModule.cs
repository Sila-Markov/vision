    ...
    class LaboratoryModule : GVMP.Module.Module<LaboratoryModule>
    {
		...
		public void enterLaboratory(Client c, int Id)
		{
			try
			{
				...
				Laboratory laboratory = laboratories.Find((Laboratory labor) => labor.Id == Id);
		...
		public void leaveLaboratory(Client c)
		{
			try
			{
				...
				Laboratory laboratory = laboratories.Find((Laboratory labor) => labor.FactionId == dbPlayer.Faction.Id);
		...
		public override void OnFiveMinuteUpdate()
		{
			try
			{
				foreach (DbPlayer dbPlayer in PlayerHandler.GetPlayers())
				{
					...
					ItemModel itemModel2 = laborStorageItems.Find((ItemModel itemModel) => itemModel.Name == "Ephidrinpulver");
					if (itemModel2 == null)
					{
						continue;
					}
					ItemModel itemModel3 = laborStorageItems.Find((ItemModel itemModel) => itemModel.Name == "Batteriezelle");
					if (itemModel3 == null)
					{
						continue;
					}
					if (itemModel3.Amount >= 5 && itemModel2.Amount >= 45)
					{
						...
						Item item3 = ItemModule.itemRegisterList.Find((Item item2) => item2.Name == "Meth");
